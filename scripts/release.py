...
def main():
    ...
    # 6. 运行全链路自动化测试套件
    test_script = base_dir / "scripts" / "run-all-tests.py"
    run_command(f"python3 {test_script}", base_dir, "Running Full-Chain Test Suites (21/21)")
    ...
    try:
        ...
        if isinstance(pack_data, list) and len(pack_data) > 0:
            files_count = pack_data[0].get("entryCount", "N/A")
            unpacked_size = pack_data[0].get("unpackedSize", "N/A")
    ...
    if args.publish:
        ...
        print("  1. Review Git status and commit changes:")
        print(f"     git add . && git commit -m \"chore(release): bump version to v{target_version}\"")
        print(f"     git tag v{target_version} && git push origin main --tags")
        print("\n  2. Publish to NPM registry:")
        print("     npm publish --access public")
        print("\n  3. Verify on NPM:")
        print(f"     npm view knowme-careerforge-skill@v{target_version}")
