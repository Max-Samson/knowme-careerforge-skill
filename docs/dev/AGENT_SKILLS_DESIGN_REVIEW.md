...
## 2026-09-06 运行回放后的增量修复

上述评审保留为历史基线。本次补齐完整安装分支的 package.json，源码安装可复制锁文件；npm 归档不含 package-lock.json，因此明确使用 npm install --omit=dev 而非声称锁定安装。安装器不再复制开发入口到新运行目录，并以非零退出报告失败。增加运行环境诊断与简短流水线报告，具体行为评审见 [真实场景回放](REAL_WORLD_SKILL_REVIEW.md)。提示适配路径和真实宿主复测仍未完成，不据工具回归声称跨平台通过。

补充：Cursor/Windsurf 安装时也准备项目内 `.knowme/skills/knowme-careerforge` 运行目录并写入准确入口路径；Gemini 配置指向同目录下安装的运行包。Windsurf 使用可更新的托管块，保留用户规则。临时目录测试覆盖六个适配安装路径；这不等于六种宿主已实际发现并正确执行。
