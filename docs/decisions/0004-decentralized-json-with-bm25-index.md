...
1. Each template maintains its own structured `metadata.json` alongside `template.html` and `style.css` (ensuring Locality and zero merge conflicts);
...
## Consequences
### Positive
- **High Locality & Modularity**: Adding a new template is strictly folder-scoped (drop folder with metadata.json, template.html, style.css);
